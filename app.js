...
async function loadChapter(){
  const fileMap={'chuong-4':'chapter4.json'}; const file=fileMap[chapterSlug]||'chapter4.json'; const r=await fetch(file); chapterData=await r.json();
...
function memeFor(p){if(p===100)return['meme-perfect.svg','QUÁ DỮ!','40/40 — bài làm sạch đẹp như giáo trình.'];if(p>=90)return['meme-great.svg','ĐỈNH NÓC!','Kiến thức đang vào form rất đẹp.'];if(p>=70)return['meme-great.svg','ỔN ÁP!','Nền tảng tốt, xem lại vài câu sai là lên điểm ngay.'];if(p>=50)return['meme-mid.svg','SUÝT ĐẸP!','Xem phần giải thích bên dưới rồi thử lại nhé.'];return['meme-retry.svg','CHƯƠNG NÀY ĐANG GỌI TÊN...','Ôn lại từng mục rồi quay lại phục thù!']}
...
function showResult(r){
  ...
  const m=memeFor(Number(r.percent));$('#meme').innerHTML=`<img src="${m[0]}" alt="Meme minh họa kết quả"><strong>${m[1]}</strong><div>${m[2]}</div>`;
